...
struct SettingsView: View {
    ...
    func testConnection(fetchModelsOnSuccess: Bool = false) {
        ...
        // ---> Trim address here too, just in case it was needed before <---
        ...
        guard var components = URLComponents(string: "http://\(trimmedAddress)") else { // Use trimmed
            self.connectionStatus = "Error: Invalid server address format."
            isTestingConnection = false
            return
        }
        components.port = portInt
        components.path = "/api/version" // Use /api/version (more standard)
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            DispatchQueue.main.async {
                ...
                // Handle 404 for older Ollama without /api/version
                if httpResponse.statusCode == 404 {
                   self.connectionStatus = "Connected (Version N/A)"
                   if fetchModelsOnSuccess { fetchModels() }
                   return
                }
    ...
    func fetchModels() {
        ...
        guard let portInt = Int(ollamaPort), portInt > 0 && portInt <= 65535 else {
             // Don't change status, just log or handle silently
            print("Error: Invalid port for fetching models.")
            isFetchingModels = false
            return
        }
         // ---> Trim address here too <---
        let trimmedAddress = ollamaAddress.trimmingCharacters(in: .whitespacesAndNewlines)
         guard !trimmedAddress.isEmpty else {
            print("Error: Server address cannot be empty when fetching models.")
            isFetchingModels = false
            return
        }
        guard var components = URLComponents(string: "http://\(trimmedAddress)") else { // Use trimmed
            print("Error: Invalid server address for fetching models.")
            isFetchingModels = false
            return
        }
        components.port = portInt
        ...
        guard let url = components.url else {
             print("Error: Could not construct URL for models.")
             isFetchingModels = false
             return
        }
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            DispatchQueue.main.async {
                ...
                do {
                    ...
                    print("Error: Could not decode models. \(error.localizedDescription)")
